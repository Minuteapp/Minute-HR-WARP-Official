...
interface QueryRequestModalProps {
  request: {
    id: string;
...
export const QueryRequestModal = ({ request, open, onOpenChange }: QueryRequestModalProps) => {
  const [question, setQuestion] = useState('');
  const queryClient = useQueryClient();
  const typeBadge = getTypeBadge(request.type);

  const queryMutation = useMutation({
    mutationFn: (data: { requestId: string, question: string }) => 
      absenceService.sendQueryToEmployee(data.requestId, data.question),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          ...
          {/* Question Input */}
          <div>
            <label className="text-sm font-medium mb-2 block">Ihre Frage</label>
            <Textarea
          ...
          {/* Info */}
          <div className="flex gap-2 text-xs text-muted-foreground bg-blue-50 p-3 rounded-lg">
            <Info className="h-4 w-4 text-blue-600 flex-shrink-0 mt-0.5" />
            <p>Der Mitarbeiter wird per E-Mail über Ihre Rückfrage benachrichtigt und kann darauf antworten.</p>
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={queryMutation.isPending}>
            Abbrechen
          </Button>
          <Button onClick={handleSubmit} disabled={queryMutation.isPending}>
            {queryMutation.isPending ? 'Wird gesendet...' : 'Rückfrage senden'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
