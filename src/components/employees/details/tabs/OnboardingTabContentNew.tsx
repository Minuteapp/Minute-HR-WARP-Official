...
import { OnboardingTaskDialog } from '../dialogs/OnboardingTaskDialog';
import { CheckSquare, Flag, Laptop, Users, Plus, Edit, Trash2 } from 'lucide-react';

interface OnboardingTabContentNewProps {
  employeeId: string;
}

export const OnboardingTabContentNew: React.FC<OnboardingTabContentNewProps> = ({ employeeId }) => {
  const { checklists, milestones, equipment, buddy, isLoading, toggleChecklistItem } = useOnboardingData(employeeId);
  const { canCreate, canEdit } = useEnterprisePermissions();
  ...
  const handleCreate = async (data: any) => {
    // Onboarding task creation logic would go here
    console.log('Create onboarding task:', data);
  };

  const handleEdit = async (data: any) => {
    // Onboarding task update logic would go here
    console.log('Update onboarding task:', data);
  };
        ...
        <CardContent>
          {checklists && checklists.length > 0 ? (
              ...
              {checklists.map((item) => (
                <div key={item.id} className="flex items-start gap-3 p-3 border rounded-lg">
                  <Checkbox
                    ...
                    className="mt-0.5"
                    disabled={!canEdit('employee_onboarding')}
                  />
                  <div className="flex-1">
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>Keine Checklisten-Punkte vorhanden</p>
              {canCreate('employee_onboarding') && (
