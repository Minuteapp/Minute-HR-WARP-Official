...
export const MyGoalsTab = () => {
  ...
  const loadGoals = async () => {
    try {
      setLoading(true);
      const data = await goalsStatisticsService.getMyGoals('current-user');
      setGoals(data);
    } catch (error) {
      ...
      {filteredGoals.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <h3 className="text-xl font-semibold text-foreground mb-2">
            Keine persönlichen Ziele gefunden
          </h3>
          <p className="text-sm text-muted-foreground mb-6">
            Erstellen Sie Ihr erstes persönliches Ziel
          </p>
          <Button>
