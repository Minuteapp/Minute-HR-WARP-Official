-- Erweiterte Rechtematrix-Tabellen (ohne bereits existierende Enums)

-- Prüfe ob notification_type bereits existiert
DO $$ 
BEGIN
...
-- Permission Modules Policies
DROP POLICY IF EXISTS "Admins can manage permission modules" ON public.permission_modules;
...
DROP POLICY IF EXISTS "All can view active permission modules" ON public.permission_modules;
CREATE POLICY "All can view active permission modules"
ON public.permission_modules
FOR SELECT
USING (is_active = true);

-- Role Permission Matrix Policies
DROP POLICY IF EXISTS "Admins can manage role permission matrix" ON public.role_permission_matrix;
...
DROP POLICY IF EXISTS "Admins can view permission audit log" ON public.permission_audit_log;
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(rpm.allowed_actions, ARRAY[]::TEXT[]) as allowed_actions,
    COALESCE(rpm.visible_fields, '{}'::jsonb) as visible_fields,
    COALESCE(rpm.editable_fields, '{}'::jsonb) as editable_fields,
    COALESCE(rpm.allowed_notifications, ARRAY[]::TEXT[]) as allowed_notifications,
