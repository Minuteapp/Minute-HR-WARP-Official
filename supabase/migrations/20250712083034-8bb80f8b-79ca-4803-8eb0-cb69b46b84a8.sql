-- Erweitere die Roadmap-Planungsstrukturen für die Container-Details und Sub-Container
...
-- Erweitere Container-Tabelle um zusätzliche Felder
ALTER TABLE public.roadmap_planning_containers 
ADD COLUMN IF NOT EXISTS description TEXT,
ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'planned' CHECK (status IN ('planned', 'in_progress', 'done', 'blocked')),
...
ADD COLUMN IF NOT EXISTS font_size INTEGER DEFAULT 14 CHECK (font_size >= 8 AND font_size <= 72),
ADD COLUMN IF NOT EXISTS has_sub_containers BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS sub_containers_visible BOOLEAN DEFAULT true,
...
-- RLS Policies für team_members (ähnlich wie bei tasks)
