...
-- RLS Policies für roadmap_planning_boards
CREATE POLICY "Users can view boards from accessible roadmaps" ON public.roadmap_planning_boards
  FOR SELECT USING (
    roadmap_id IN (
      SELECT id FROM public.roadmaps 
      WHERE created_by = auth.uid() OR id IN (
        SELECT roadmap_id FROM public.roadmap_collaborators 
...
CREATE POLICY "Users can view containers from accessible boards" ON public.roadmap_planning_containers
  FOR SELECT USING (
    board_id IN (
      ...
      WHERE r.created_by = auth.uid() OR r.id IN (
        SELECT roadmap_id FROM public.roadmap_collaborators 
        WHERE user_id = auth.uid()
      )
    )
