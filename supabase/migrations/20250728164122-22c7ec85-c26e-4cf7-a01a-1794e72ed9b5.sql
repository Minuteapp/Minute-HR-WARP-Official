-- FINALE DATENISOLATION - Korrekte Spalten verwenden
-- Behebt das kritische Problem: Neue Firmen sehen fremde Daten

-- 1. RLS für TASKS Tabelle
...
-- 5. RLS für DOCUMENTS Tabelle (hat employee_id statt company_id!)
...
CREATE POLICY "Company Isolation for Documents" ON documents
  ...
  USING (
    ...
    (is_superadmin_safe(auth.uid()) AND NOT is_in_tenant_context())
    OR
    -- Im Tenant-Kontext: nur Dokumente von Mitarbeitern der Tenant-Firma
    ...
    OR
    -- Normale Benutzer: nur eigene Dokumente oder von ihrer Firma
    (NOT is_in_tenant_context() AND (
      created_by = auth.uid()
