-- Innovation Hub: Enterprise-ready Datenmodell
...
CREATE TABLE public.innovation_ideas (
  ...
  status TEXT NOT NULL DEFAULT 'new' CHECK (status IN ('new', 'under_review', 'approved', 'in_development', 'pilot_phase', 'implemented', 'rejected', 'archived')),
  priority TEXT NOT NULL DEFAULT 'medium' CHECK (priority IN ('low', 'medium', 'high')),
  tags TEXT[] DEFAULT '{}',
  attachments TEXT[] DEFAULT '{}',
  
  -- Neue Enterprise-Felder
  ...
  -- KI-Bewertung
  ai_feasibility_score DECIMAL,
  ai_cost_estimate DECIMAL,
  ai_strategic_relevance DECIMAL,
  ai_roi_prediction DECIMAL,
  ai_analysis_pros TEXT[],
  ai_analysis_cons TEXT[],
  ai_last_analyzed TIMESTAMP WITH TIME ZONE,
  
  -- Visualisierung
  visualization_x DECIMAL DEFAULT 0,
  visualization_y DECIMAL DEFAULT 0,
  visualization_z DECIMAL DEFAULT 0,
  parent_idea_id UUID REFERENCES public.innovation_ideas(id),
...
CREATE TABLE public.innovation_campaigns (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  category TEXT,
...
-- KI-Insights
CREATE TABLE public.innovation_ai_insights (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  idea_id UUID NOT NULL REFERENCES public.innovation_ideas(id) ON DELETE CASCADE,
...
CREATE TABLE public.innovation_analytics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  company_id UUID NOT NULL,
  campaign_id UUID REFERENCES public.innovation_campaigns(id),
...
CREATE TABLE public.innovation_user_points (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  user_name TEXT NOT NULL,
  campaign_id UUID REFERENCES public.innovation_campaigns(id),
...
CREATE TABLE public.innovation_project_templates (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  category TEXT,
...
-- Lifecycle Tracking
CREATE TABLE public.innovation_lifecycle_tracking (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  idea_id UUID NOT NULL REFERENCES public.innovation_ideas(id) ON DELETE CASCADE,
  roadmap_id UUID,
  project_id UUID,
...
CREATE POLICY "Admins can update all ideas in their company" ON public.innovation_ideas
  FOR UPDATE USING (company_id = get_user_company_id(auth.uid()) AND is_admin_safe(auth.uid()));
...
CREATE INDEX idx_innovation_ideas_company_id ON public.innovation_ideas(company_id);
CREATE INDEX idx_innovation_ideas_status ON public.innovation_ideas(status);
CREATE INDEX idx_innovation_ideas_created_by ON public.innovation_ideas(created_by);
CREATE INDEX idx_innovation_ideas_parent ON public.innovation_ideas(parent_idea_id);
CREATE INDEX idx_innovation_campaigns_company_id ON public.innovation_campaigns(company_id);
CREATE INDEX idx_innovation_votes_idea_id ON public.innovation_votes(idea_id);
CREATE INDEX idx_innovation_comments_idea_id ON public.innovation_comments(idea_id);
CREATE INDEX idx_innovation_ai_insights_idea_id ON public.innovation_ai_insights(idea_id);

-- Triggers
CREATE OR REPLACE FUNCTION update_innovation_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
