-- Retry migration with policy creation wrapped to avoid IF NOT EXISTS syntax issues
-- 1) ENUMs
DO $$ BEGIN
...
-- 13) Validation trigger for assignments
...
-- 15) Policies via DO blocks
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='turbine_positions' AND policyname='turbine_positions_admin_all') THEN
    CREATE POLICY "turbine_positions_admin_all" ON public.turbine_positions
      FOR ALL USING (
        EXISTS (
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='shift_position_slots' AND policyname='shift_slots_admin_all') THEN
    CREATE POLICY "shift_slots_admin_all" ON public.shift_position_slots
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='employee_qualifications' AND policyname='employee_qual_admin_all') THEN
    CREATE POLICY "employee_qual_admin_all" ON public.employee_qualifications
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='position_assignments' AND policyname='assignments_admin_all') THEN
    CREATE POLICY "assignments_admin_all" ON public.position_assignments
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='open_shift_bids' AND policyname='bids_admin_all') THEN
    CREATE POLICY "bids_admin_all" ON public.open_shift_bids
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='swap_requests' AND policyname='swaps_admin_all') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='business_trip_approvals' AND policyname='approvals_admin_all') THEN
    CREATE POLICY "approvals_admin_all" ON public.business_trip_approvals
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='travel_documents' AND policyname='travel_docs_admin_all') THEN
    CREATE POLICY "travel_docs_admin_all" ON public.travel_documents
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='expense_ocr_queue' AND policyname='ocr_queue_admin_all') THEN
    CREATE POLICY "ocr_queue_admin_all" ON public.expense_ocr_queue
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='mileage_calculations' AND policyname='mileage_admin_all') THEN
    CREATE POLICY "mileage_admin_all" ON public.mileage_calculations
      FOR ALL USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')))
      WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role IN ('admin','hr','superadmin')));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='employee_qualifications' AND policyname='qual_owner_read') THEN
    CREATE POLICY "qual_owner_read" ON public.employee_qualifications
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='position_assignments' AND policyname='assign_owner_read') THEN
    CREATE POLICY "assign_owner_read" ON public.position_assignments
      FOR SELECT USING (user_id = auth.uid());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='open_shift_bids' AND policyname='bids_owner_crud') THEN
    CREATE POLICY "bids_owner_crud" ON public.open_shift_bids FOR SELECT USING (user_id = auth.uid());
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='open_shift_bids' AND policyname='bids_owner_insert') THEN
    CREATE POLICY "bids_owner_insert" ON public.open_shift_bids FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='open_shift_bids' AND policyname='bids_owner_update') THEN
    CREATE POLICY "bids_owner_update" ON public.open_shift_bids FOR UPDATE USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='swap_requests' AND policyname='swaps_owner_insert') THEN
    CREATE POLICY "swaps_owner_insert" ON public.swap_requests
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='swap_requests' AND policyname='swaps_owner_read') THEN
    CREATE POLICY "swaps_owner_read" ON public.swap_requests
      FOR SELECT USING (
        auth.uid() = (
          SELECT pa.user_id FROM public.position_assignments pa
          WHERE pa.id = requester_assignment_id
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='business_trip_approvals' AND policyname='approvals_approver_read_update') THEN
    CREATE POLICY "approvals_approver_read_update" ON public.business_trip_approvals
      FOR SELECT USING (approver_id = auth.uid());
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='business_trip_approvals' AND policyname='approvals_approver_update') THEN
    CREATE POLICY "approvals_approver_update" ON public.business_trip_approvals
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='travel_documents' AND policyname='travel_docs_owner_read') THEN
    CREATE POLICY "travel_docs_owner_read" ON public.travel_documents
      FOR SELECT USING (user_id = auth.uid());
  END IF;
END $$;

-- Optional: OCR queue owner read (abhängig von business_trip_expenses.user_id)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class WHERE relname='business_trip_expenses'
  ) THEN
    -- skip owner policy if expense table nicht existiert
    NULL;
  ELSE
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='expense_ocr_queue' AND policyname='ocr_queue_owner_read'
    ) THEN
      CREATE POLICY "ocr_queue_owner_read" ON public.expense_ocr_queue
        FOR SELECT USING (
          auth.uid() = (
