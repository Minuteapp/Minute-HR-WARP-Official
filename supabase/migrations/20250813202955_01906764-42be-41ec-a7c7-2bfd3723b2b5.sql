-- Advanced Time Tracking System Migration
...
CREATE INDEX IF NOT EXISTS idx_time_entries_user_date ON public.time_entries(user_id, start_time::date);
...
-- Automatische Überstunden-Berechnung Trigger
CREATE OR REPLACE FUNCTION public.calculate_overtime()
RETURNS TRIGGER AS $$
DECLARE
  daily_threshold NUMERIC(5,2);
  overtime_multiplier NUMERIC(3,2);
BEGIN
  -- Hole Überstunden-Regel für den Benutzer
  SELECT or.daily_threshold_hours, or.overtime_multiplier
  INTO daily_threshold, overtime_multiplier
  FROM public.overtime_rules or
  JOIN public.time_tracking_settings tts ON tts.overtime_rule_id = or.id
  WHERE tts.user_id = NEW.user_id 
  AND or.is_active = true
  AND CURRENT_DATE BETWEEN or.effective_from AND COALESCE(or.effective_until, '2099-12-31')
  LIMIT 1;

  -- Fallback auf Standard-Werte
  daily_threshold := COALESCE(daily_threshold, 8.0);
  overtime_multiplier := COALESCE(overtime_multiplier, 1.5);

  -- Berechne Überstunden nur wenn end_time gesetzt ist
  IF NEW.end_time IS NOT NULL AND NEW.total_hours IS NOT NULL THEN
    NEW.overtime_hours := GREATEST(0, NEW.total_hours - daily_threshold);
  END IF;
...
CREATE TRIGGER calculate_overtime_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION public.calculate_overtime();
