-- Performance Management System Migration
...
CREATE OR REPLACE FUNCTION auto_assign_company_performance()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER performance_cycles_company_trigger
  BEFORE INSERT ON public.performance_cycles
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER performance_templates_company_trigger
  BEFORE INSERT ON public.performance_templates
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER performance_reviews_company_trigger
  BEFORE INSERT ON public.performance_reviews
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER performance_feedback_company_trigger
  BEFORE INSERT ON public.performance_feedback
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER pulse_questions_company_trigger
  BEFORE INSERT ON public.pulse_questions
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER calibration_matrix_company_trigger
  BEFORE INSERT ON public.calibration_matrix
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER development_plans_company_trigger
  BEFORE INSERT ON public.development_plans
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER succession_plans_company_trigger
  BEFORE INSERT ON public.succession_plans
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

CREATE TRIGGER performance_audit_trail_company_trigger
  BEFORE INSERT ON public.performance_audit_trail
  FOR EACH ROW EXECUTE FUNCTION auto_assign_company_performance();

-- Updated trigger
CREATE OR REPLACE FUNCTION update_updated_at_performance()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER performance_cycles_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_performance();

CREATE TRIGGER performance_templates_updated_at
...
CREATE TRIGGER succession_plans_updated_at
...
CREATE POLICY "performance_cycles_select" ON public.performance_cycles
  FOR SELECT USING (company_id = get_user_company_id(auth.uid()) OR is_superadmin_safe(auth.uid()));

CREATE POLICY "performance_cycles_insert" ON public.performance_cycles
  FOR INSERT WITH CHECK (
    (company_id = get_user_company_id(auth.uid()) OR is_superadmin_safe(auth.uid())) AND
...
CREATE POLICY "performance_templates_update" ON public.performance_templates
  FOR UPDATE USING (
    (company_id = get_user_company_id(auth.uid()) AND created_by = auth.uid()) OR
    (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr', 'superadmin'))) OR
    is_superadmin_safe(auth.uid())
  );
...
CREATE POLICY "performance_reviews_select" ON public.performance_reviews
  FOR SELECT USING (
    ...
    (company_id = get_user_company_id(auth.uid()) AND EXISTS (
...
CREATE POLICY "performance_feedback_select" ON public.performance_feedback
  FOR SELECT USING (
    ...
    (visibility IN ('manager', 'hr', 'public') AND company_id = get_user_company_id(auth.uid())) OR
    (company_id = get_user_company_id(auth.uid()) AND EXISTS (
      SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr', 'manager', 'superadmin')
    )) OR
    is_superadmin_safe(auth.uid())
...
CREATE POLICY "development_plans_select" ON public.development_plans
  FOR SELECT USING (
    ...
    (company_id = get_user_company_id(auth.uid()) AND EXISTS (
      SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr', 'superadmin')
    )) OR
    is_superadmin_safe(auth.uid())
  );
...
-- Helper function to get user company ID (if it doesn't exist)
