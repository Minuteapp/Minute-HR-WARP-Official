-- Erweiterte Spesenabrechnung für Business Travel (korrigiert)
...
-- Vereinfachte RLS Policies (ohne company_id-Abhängigkeiten)
CREATE POLICY "Users can view expense categories" ON public.expense_categories FOR SELECT
  USING (true);

CREATE POLICY "Users can view per diem rates" ON public.per_diem_rates FOR SELECT
  USING (true);

CREATE POLICY "Users can view mileage rates" ON public.mileage_rates FOR SELECT
  USING (true);
...
CREATE POLICY "Users can view company cards" ON public.company_cards FOR SELECT
  USING (assigned_to = auth.uid() OR EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr')));

CREATE POLICY "Users can view card transactions" ON public.card_transactions FOR SELECT
  USING (company_card_id IN (SELECT id FROM public.company_cards WHERE assigned_to = auth.uid()) OR 
         EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr')));

CREATE POLICY "Users can view document workflows" ON public.document_workflows FOR SELECT
  USING (document_id IN (SELECT id FROM documents WHERE created_by = auth.uid()) OR 
         assigned_to = auth.uid() OR
         EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr')));

CREATE POLICY "Users can manage their document workflows" ON public.document_workflows FOR INSERT
  WITH CHECK (assigned_to = auth.uid() OR 
               EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('admin', 'hr')));

CREATE POLICY "Users can update document workflows" ON public.document_workflows FOR UPDATE
...
CREATE INDEX IF NOT EXISTS idx_per_diem_rates_country ON public.per_diem_rates(country_code, effective_from);
CREATE INDEX IF NOT EXISTS idx_mileage_rates_vehicle ON public.mileage_rates(vehicle_type, effective_from);
CREATE INDEX IF NOT EXISTS idx_card_transactions_card ON public.card_transactions(company_card_id, transaction_date);
CREATE INDEX IF NOT EXISTS idx_document_workflows_status ON public.document_workflows(status, assigned_to);