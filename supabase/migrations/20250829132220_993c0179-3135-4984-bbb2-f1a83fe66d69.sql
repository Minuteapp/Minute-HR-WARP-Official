...
CREATE POLICY "Users can view their company budget plans" ON public.budget_plans
FOR SELECT USING (
...
CREATE POLICY "Admins can manage budget plans" ON public.budget_plans
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.user_roles ur 
    WHERE ur.user_id = auth.uid() 
...
CREATE POLICY "Users can view their company budget templates" ON public.budget_templates
FOR SELECT USING (
  company_id = get_user_company_id(auth.uid()) OR
  EXISTS (
    SELECT 1 FROM public.user_roles ur 
...
CREATE POLICY "Users can view their company budget forecasts" ON public.budget_forecasts
FOR SELECT USING (
  company_id = get_user_company_id(auth.uid()) OR
  EXISTS (
    SELECT 1 FROM public.user_roles ur 
...
-- Create auto-assignment trigger for company_id
CREATE OR REPLACE TRIGGER auto_assign_budget_plans_company_id
  BEFORE INSERT ON public.budget_plans
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_assign_company_id();

CREATE OR REPLACE TRIGGER auto_assign_budget_templates_company_id
  BEFORE INSERT ON public.budget_templates
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_assign_company_id();

CREATE OR REPLACE TRIGGER auto_assign_budget_forecasts_company_id
  BEFORE INSERT ON public.budget_forecasts
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_assign_company_id();